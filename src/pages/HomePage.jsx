...
function HomePage() {
  ...
  const [dailyStatusByDate, setDailyStatusByDate] = useState(() => loadJSON(STORAGE_KEYS.dailyStatus, {}));
  const todayKey = getTodayKey();
  const todayStatus = dailyStatusByDate[todayKey] || {};
  ...
  useEffect(() => {
    saveJSON(STORAGE_KEYS.dailyStatus, dailyStatusByDate);
  }, [dailyStatusByDate]);

  const handleUpdateStatus = (planId, time, status) => {
    setDailyStatusByDate((prev) => ({
      ...prev,
      [todayKey]: {
        ...(prev[todayKey] || {}),
        [planId]: {
          ...((prev[todayKey] || {})[planId] || {}),
          [time]: status,
        },
      },
    }));
  ...
  const metrics = useMemo(() => {
    ...
    const completedTasks = allTasks.filter(({ planId, time }) => todayStatus[planId]?.[time] === STATUS_VALUE.taken).length;
    const skippedTasks = allTasks.filter(({ planId, time }) => todayStatus[planId]?.[time] === STATUS_VALUE.skipped).length;
  ...
  }, [activeSection, metrics, plans, reminders, todayStatus]);
